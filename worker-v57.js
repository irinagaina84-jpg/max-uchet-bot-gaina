...
const WORKER_VERSION = "worker-v57-ledger-backfill-gate";
...
async function ensureV57(runtimeEnv) {
...
export default {
  async fetch(request, runtimeEnv, ctx) {
    ...
    const container = await ensureV57(runtimeEnv);
    ...
    if (url.pathname === "/diagnostic") {
      ...
      ctx.waitUntil(containerHealth(container).catch((error) => console.error("v57 health start error", error)));
  ...
  async scheduled(_controller, runtimeEnv, ctx) {
    ...
    })().catch((error) => console.error("v57 keepalive error", error)));
  },
};
