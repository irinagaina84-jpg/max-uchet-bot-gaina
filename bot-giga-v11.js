    ...
    const req = https.request({
      ...
      res.on("end", () => {
        ...
        if (status >= 400) {
          ...
          const error = new Error(`${status}: ${typeof detail === "string" ? detail : JSON.stringify(detail)}`);
          error.status = status;
          error.data = data;
          reject(error);
          return;
    ...
    const req = client.request({
      ...
      res.on("data", (chunk) => {
        ...
        if (size > 20 * 1024 * 1024) {
          req.destroy(new Error("image is larger than 20 MB"));
        } else {
          chunks.push(chunk);
        }
      });
...
async function normalizeImageForGiga(buffer, contentType) {
  const mime = String(contentType || "").toLowerCase();
  state.lastImageMime = mime || null;

  const supported = new Set(["image/jpeg", "image/png", "image/tiff", "image/bmp"]);
  if (supported.has(mime)) {
    state.lastConvertedImageMime = mime;
  ...
  try {
    const converted = await sharp(buffer, { animated: false })
      ...
      .toBuffer();
    state.lastConvertedImageMime = "image/jpeg";
    return { buffer: converted, contentType: "image/jpeg" };
  } catch (error) {
    throw new Error(`Не удалось преобразовать ${mime || "изображение"} в JPEG: ${errText(error)}`);
  }
}
...
async function getGigaAccessToken(force = false) {
  ...
  for (const candidate of authCandidates()) {
    try {
      ...
      gigaAccessExpiresAtMs = expiresAt > 1e12
        ? expiresAt
        : expiresAt > 1e9
          ? expiresAt * 1000
          : Date.now() + 29 * 60 * 1000;
      ...
      errors.push(`${candidate.mode}: ${errText(error)}`);
      state.lastGigaStatus = error?.status || null;
    }
  }
...
async function uploadImageToGiga(inputBuffer, inputContentType = "image/jpeg") {
  const normalized = await normalizeImageForGiga(inputBuffer, inputContentType);
  const buffer = normalized.buffer;
  const contentType = normalized.contentType;

  if (buffer.length > 15 * 1024 * 1024) {
    throw new Error("Изображение после преобразования больше 15 МБ");
  }
...
async function analyzeGroupImages(chatId, name, urls) {
  const context = (groupContext.get(String(chatId)) || [])
    .slice(-25)
    .map((entry) => `${entry.time || ""} ${entry.sender || ""}: ${entry.text || "[изображение]"}`)
    .join("\n");
...
async function interpretCorrection(chatId, name, text) {
  const context = (groupContext.get(String(chatId)) || [])
    .slice(-25)
    .map((entry) => `${entry.sender || ""}: ${entry.text || "[изображение]"}`)
    .join("\n");
...
async function handleGroupMessage(message) {
  ...
  // В групповых чатах бот всегда молчит.
  if (urls.length) {
    ...
    try {
      ...
      state.lastError = `Group image analysis: ${errText(error)}`;
      await sendPrivateReport(`Не смог прочитать картинку из «${name}»: ${error.message}`).catch(() => {});
    }
    return;
  }
  ...
  if (text && CORRECTION_RE.test(text) && reportUserId) {
    try {
      ...
      if (analysis && !/^учет не меняет[.!]?$/i.test(analysis)) {
        await sendPrivateReport(`Изменение в «${name}»\n\n${analysis}`);
      }
    } catch (error) {
      state.lastError = `Correction analysis: ${errText(error)}`;
    }
  }
}
...
async function handlePrivateMessage(message) {
  ...
  try {
    ...
    state.lastError = `Private processing: ${errText(error)}`;
    await sendToUser(senderId, `Ошибка обработки: ${error.message}`).catch(() => {});
  }
}
...
async function pollForever() {
  ...
  while (true) {
    try {
      ...
      if (!first && marker != null) params.set("marker", String(marker));

      const data = await maxRequest(`/updates?${params}`, { timeout: 40000 });
      first = false;
      ...
      state.lastError = `Polling: ${errText(error)}`;
      await sleep(3000);
    }
  }
}
...
async function start() {
  while (true) {
    try {
      ...
      try {
        await getGigaAccessToken(false);
        state.lastError = null;
      } catch (error) {
        state.lastError = `GigaChat auth: ${errText(error)}`;
      }
      ...
      state.polling = false;
      state.lastError = `Startup: ${errText(error)}`;
      await sleep(5000);
    }
  }
}
