...
function replaceRegex(pattern, replacement, label, required = true) {
  if (!pattern.test(code)) {
    if (required) throw new Error(`text-release patch anchor not found: ${label}`);
    console.log(`text-release skipped: ${label}`);
    return;
  }
...
const replyInfoNew = String.raw`function linkedMessageText(m) {
  const candidates = [
    m?.body?.reply_to,
    m?.body?.reply,
    m?.reply_to,
...
const seriesNew = String.raw`function namedReleaseSeries(text) {
...
const helpers = String.raw`function terminalFromText(text) {
...
const historyReplacement = String.raw`function historyContextLine(m) {
...
replaceRegex(
  /function historyContextLine\(m\) \{[\s\S]*?\n\}/,
  historyReplacement,
  "add deterministic hints to transcript"
);

replaceOnce(
  "Поздние реплики могут отменять или исправлять ранние.",
  "Поздние реплики могут отменять или исправлять ранние. Цитаты и reply-цепочки относятся к конкретному сообщению; короткие текстовые релизы и терминальные коды читай вместе с соседним контекстом.",
  "semantic extraction reminder"
);
...
replaceOnce(
  ...
  '8) Если пользователь называет клиента (например «по Взлёту»), фильтруй события по явной или надёжно следующей из переписки привязке к этому клиенту. Если пользователь спрашивает про именованную серию вида «Релиз Ирина», «Релиз Матвейченкова» и т.п., фильтруй по release_id и notes с series=<название>; собери ВСЕ события этой серии, включая картинки и текстовые сообщения без повторения имени. Учитывай текстовые формы «Чехов 10 релиз Ирина», reply-цепочки и компактные записи вида GFM-10x20DC/p.',
  "final answer uses text release patterns"
);

code = code.replace(/version:\s*"v[^"]+",/, 'version: "v53-text-release-logic",');
if (code.includes('namedReleaseSeriesFixed: true,') && !code.includes('textReleaseLogicFixed: true,')) {
