...
import stableWorker, { MaxBotContainer } from "./worker-v71.js";

const CONTAINER_INSTANCE = "production";
const DEFAULT_CHAT_ID = "-77828005225953";
const WORKER_VERSION = "worker-v72-multichat-durable-export";
...
function normalizeChatId(value) {
  let chatId = String(value || DEFAULT_CHAT_ID).trim();
  if (/^\d{10,20}$/.test(chatId)) chatId = `-${chatId}`;
  if (!/^-?\d{1,20}$/.test(chatId)) throw new Error("Invalid chat_id");
  return chatId;
}
...
async function loadRawRecords(runtimeEnv, chatId, from, to, requestedLimit) {
  ...
  for (let page = 0; page < 30 && records.length < hardLimit; page += 1) {
    ...
    const result = await stub.rawRangePage(chatId, from, to, cursor, size);
  ...
  for (const record of records) {
    ...
    if (!previous || Number(record?.saved_at || 0) >= Number(previous?.saved_at || 0)) {
      byMid.set(key, record);
    }
  }
...
function attachmentAsset(attachment, attachmentIndex) {
  ...
  for (const candidate of state.urls) {
...
function crc32(bytes) {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    c = CRC32_TABLE[(c ^ bytes[index]) & 0xff] ^ (c >>> 8);
  }
...
async function* archiveGenerator(runtimeEnv, records, chatId, from, to) {
  let offset = 0;
  const central = [];
  const manifest = [[
  ...
  yield* addEntry("history.txt", encoder.encode(historyText(records, chatId, from, to)));
  ...
  const readme = [
    "MAX — выгрузка выбранного рабочего чата",
    `Версия: ${WORKER_VERSION}`,
    `Чат: ${chatId}`,
    `Период: ${from ? localDate(from) : "начало"} — ${to ? localDate(to - 1) : "сейчас"}`,
    `Сообщений: ${records.length}`,
...
async function exportSelectedChat(request, runtimeEnv) {
  ...
  const chatId = normalizeChatId(url.searchParams.get("chat_id") || DEFAULT_CHAT_ID);
  ...
  const records = await loadRawRecords(runtimeEnv, chatId, from, to, limit);
  const suffix = from || to
    ? `${from ? localDate(from) : "start"}_${to ? localDate(to - 1) : "now"}`
    : `all_${localDate(Date.now())}`;
  const filename = `max_chat_${safePart(chatId.replace(/^-/, ""), "chat")}_${suffix}.zip`;

  return new Response(streamFromGenerator(archiveGenerator(runtimeEnv, records, chatId, from, to)), {
    status: 200,
    headers: {
      ...
      "X-MAX-Chat-Id": chatId,
      "X-MAX-Messages": String(records.length),
    },
  });
}

async function exportHealth(runtimeEnv, request) {
  const url = new URL(request.url);
  const chatId = normalizeChatId(url.searchParams.get("chat_id") || DEFAULT_CHAT_ID);
  const page = await containerHandle(runtimeEnv).rawRangePage(chatId, 0, 0, "", 1);
...
export default {
  async fetch(request, runtimeEnv, ctx) {
    ...
    if (url.pathname === "/export/health") {
      try {
        return await exportHealth(runtimeEnv, request);
      } catch (error) {
    ...
    if (url.pathname === "/export/media" || url.pathname === "/export/saved") {
      try {
        return await exportSelectedChat(request, runtimeEnv);
      } catch (error) {
        return new Response(`Saved export error: ${error?.message || error}`, { status: 500 });
      }
    }
    return stableWorker.fetch(request, runtimeEnv, ctx);
  },

  async scheduled(controller, runtimeEnv, ctx) {
    if (typeof stableWorker.scheduled === "function") {
      return stableWorker.scheduled(controller, runtimeEnv, ctx);
    }
    return undefined;
  },
};
